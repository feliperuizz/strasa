<x-app-layout title="Editar Projeto" :client="$project->client" :project="$project">
    <x-slot name="header">
        <div class="flex items-center gap-2">
            <a href="{{ route('clients.show', $project->client) }}" class="text-sm text-slate-400 hover:text-slate-200">{{ $project->client->name }}</a>
            <span class="text-slate-600">/</span>
            <a href="{{ route('projects.board', $project) }}" class="text-sm text-slate-400 hover:text-slate-200">{{ $project->name }}</a>
            <span class="text-slate-600">/</span>
            <h1 class="text-base font-semibold text-slate-200">Editar Projeto</h1>
        </div>
    </x-slot>
                ...
                <form method="POST" action="{{ route('projects.update', $project) }}" class="space-y-4">
                    ...
                    <div>
                        ...
                        <textarea name="description" rows="3"
                                  class="w-full rounded-lg border border-ink-600 bg-ink-900 px-3 py-2 text-sm text-slate-200 focus:border-brand-500 focus:outline-none">{{ old('description', $project->description) }}</textarea>
                    ...
                    <div class="pt-4 flex items-center justify-between border-t border-ink-600">
                        ...
                        <div class="flex items-center gap-3">
                            <a href="{{ route('projects.board', $project) }}" class="text-sm text-slate-300 hover:text-slate-200">Cancelar</a>
        ...
        @can('delete', $project)
            ...
            <div x-show="showDeleteModal" style="display: none;" class="fixed inset-0 z-50 flex items-center justify-center p-4">
                ...
                <div x-show="showDeleteModal" 
                    ...
                    <h3 class="text-lg font-bold text-slate-200">Excluir Quadro Permanentemente</h3>
